...
import { DbDataLikeNew, DbDataDetail, DbDataDelete } from "./api.js";
...
export default {
  ...
  methods: {
    ...
    fetchData(
      ...
      if (!this.searchType) {
        DbDataLikeNew(postData, size, current, this.form.multiCondition.searchText).then((res) => {
          if (res.data && res.data.rows) {
            this.ztList = this.handlerData(res.data.rows);
