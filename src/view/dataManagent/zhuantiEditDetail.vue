<template>
  <div class="zhuanti-edit-detail">
    ...
    <div class="nr-line" v-show="currentType">
      <div class="zt-line" v-show="ztList">
        重复专题：
        <span class="zt-item" v-for="(item, index) in ztList" :key="index">
          <router-link tag="a" target="_blank" :to="{name:'ztDetail',query:{id:item.id}}">{{ item.titleCn }}</router-link>
          <i class="el-icon-close" title="删除专题" @click.stop="deleteZt(item)"></i>
        </span>
      </div>
    ...
    <div class="nr-line" v-show="!currentType">
      ...
      <div class="nr-line" v-show="currentType">
        <div class="zt-line" v-show="ztList">
          重复专题：
          <span class="zt-item" v-for="(item, index) in ztList" :key="index">
            <router-link tag="a" target="_blank" :to="{name:'ztDetail',query:{id:item.id}}">{{ item.title }}</router-link>
            <i class="el-icon-close" title="删除专题" @click.stop="deleteZt(item)"></i>
          </span>
        </div>
        <span v-html="detailData.contentCn"></span>
...
<script>
import { DbDataLike, DbDataDelete } from "./api";
export default {
  props: ["pageType", "detailData"],
  data() {
    return {
      currentType: false,
      ztList: [],
      ...
      const postData = {
        ...
        multiCondition: [
          ...
          },
          {
            name: "simHash",
            type: "LIKE",
            values: this.detailData.repeatDocs,
          },
        ],
      };
      DbDataLike(postData, 999999, 1).then((res) => {
        if (res.data && res.data.data && res.data.data.records) {
          this.ztList = res.data.data.records;
        }
      });
    },
    goback() {
      this.$parent.pageType = "list";
