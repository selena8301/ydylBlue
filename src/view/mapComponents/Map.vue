            ...
            setTimeout(()=>{
                 this.$nextTick(()=>{
                    ...
                    _this.rdsjs.forEach(item=>{
                        _this.rdsjrenders.push(new Popup(_this.map.viewer,item).setVisible(true));
                    });
                    // _this.map.setCountryAreasVisible(true);
...
.MapContainer{
    ...
    .bx-popup-ctn{
        // background: url('../../assets/image/ts/biaopai.png') no-repeat;
        // background-size: 100% 100%;
        // padding: 20px 30px 30px 20px;
            padding: 5px;
            background: rgba(1, 92, 189, 0.8);
            border: 1px solid #077af1;
            border-radius: 3px;
            box-shadow: 0 0 6px 1px #02274c;
        .text{
            padding-left: 10px;
        }
    }
